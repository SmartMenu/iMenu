...
public class DBMenu{
	...
	public Item[] getItems(String shopId){
		String sql="select item_id, item_name, item_name2, aa.plu_no, aa.unit, aa.cat_id, aa.discountable, aa.service_allow, price, set_price, image_file, e.desc1 as cat_name, e.desc2 as cat_name2, aa.is_modifier, aa.modifier, aa.item_set, aa.taxable, d.caption1, d.caption2 " +
					"from (select a.item_code as item_id, a.pos_desc1 as item_name, a.pos_desc2 as item_name2, a.plu_no as plu_no, a.unit, a.cat_id, b.price, b.set_price, a.discountable, a.service_allow, a.is_modifier, a.modifier1 as modifier, a.item_set as item_set, a.taxable as taxable " +
		...
		List<Object> ls = dbCommonUtil.query(sql, new ParseResultSetInterface(){
			...
			public List<Object> parseResult(ResultSet rs) throws SQLException {
				...
				while(rs.next()){
					...
					item.setItemDesc(rs.getString("caption1"));
					item.setItemDesc2(rs.getString("caption2"));
					item.setPluNo(rs.getString("plu_no"));
					item.setUnit(rs.getString("unit"));
					item.setDiscountable(rs.getInt("discountable"));
					item.setTaxable(rs.getInt("taxable"));
